...
#w2v_list = SV.use_database('../model/NTCIR_STC_ja_data_rm-at.head10k.txt', sent000) #r['text'].strip())
...
if name:
    ...
    for r in rs['replies']:
        ...
        SV = w2v_dialog.sentence_vectorizer('../model/matrix50.model')
        w2v_list = SV.use_database('../model/NTCIR.wakati.u20', r_now.encode('utf-8'))
        print r['text']
        print '\n'.join(w2v_list)
...
#        elif u'-w2v' in r['text'] or u'-word2vec' in r['text']:
#            SV = w2v_dialog.sentence_vectorizer('../model/matrix50.model')
#            w2v_list = SV.use_database('../model/NTCIR_STC_ja_data_rm-at.head10k.txt', r['text'].strip().encode('utf-8'))
#            print '\n'.join(w2v_list)
#            api.send_reply(name, r['mention_id'], r['user_name'], w2v_list[0])
